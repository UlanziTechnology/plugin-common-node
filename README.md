# ulanzideck-api  node版本

## 简介
为了让开发者提升创作者的工作效率，降低插件开发门槛，我们开发了这套Ulanzideck的插件通用库 node 版本。使用该插件库，无需再次开发socket连接，开发者只需关注插件本身所需要实现的功能，根据上位机的事件触发即可。
...
6. 由于我们的插件数量不定，为了避免端口冲突，在使用node来做主服务时，端口请通过 ulanzideck-api 提供的 RandomPort 来生成。具体可查看[<a href="#title-2">2. 生成随机端口</a>]
